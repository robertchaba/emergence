...
export const MODEL_ID = 'v4';
export const RULES_REVISION = 'v4-populations-1';
export const CONTRACT_VERSION = 'life-observations-1';
const FORMAT = 'emergence-life-v4-checkpoint-1';
...
export function createLifeModel(world, { seed = `${world.seed}:life-v4`, runId } = {}, diagnostics = {}) {
...
function buildModel(world, state, diagnostics) {
  ...
  function demography() {
    ...
    for (const residents of local.values()) {
      ...
      residents.forEach((row, index) => {
        ...
        if (rows[index].derived.sexual) state.stats.sexualBirths += births;
  ...
  function routes(row, genome) {
    ...
    const normalRate = (0.004 + 0.003 * mobility) * derived.dispersalMultiplier;
    ...
    for (const hexId of source.neighbors) {
      ...
      for (const habitat of ['land', 'water']) {
        ...
        const conductance = barrier ? normalRate * 0.006 * (1 + 0.25 * (genome.flight ?? 0)) * derived.crossingMultiplier : normalRate;
      ...
      if (environmentalPerformance(genome, environment(hexId), row.habitat, derived) <= 0) {
        for (const farId of destination.neighbors) add(farId, row.habitat,
          normalRate * 0.001 * (1 + 0.25 * (genome.flight ?? 0)) * derived.crossingMultiplier, true);
      }
    }
  ...
  function randomTrials(genome) {
    ...
    const choose = choices => {
      let ticket = random.next() * choices.reduce((sum, trial) => sum + weight(trial), 0);
      let selected = 0;
      while (selected < choices.length - 1 && ticket >= weight(choices[selected])) {
        ticket -= weight(choices[selected]); selected += 1;
      }
      return choices.splice(selected, 1)[0];
    };
    for (let index = 0; index < EVOLUTION_RULES.trialMutations && options.length; index += 1) {
      const first = choose(options);
      // Two of the existing eight slots can look across a costly intermediate.
      // Both edges are legal, seeded and distinct loci. Only the endpoint is
      // scored; the intermediate never becomes a carrier, species or free meal.
      // This permits interacting traits without prescribing a feeding transition.
      const compound = index >= EVOLUTION_RULES.trialMutations - EVOLUTION_RULES.compoundTrials;
      const next = compound ? candidateMutations(first.genome).filter(trial => trial.key !== first.key) : [];
      result.push(next.length ? { ...choose(next), steps: 2 } : { ...first, steps: 1 });
    }
    return result;
  }
  ...
  function evolve() {
    ...
    for (const record of [...state.species]) {
      ...
      for (const candidate of record.candidates) {
        ...
        // Extend a promising direction through bounded legal trials, only before
        ...
        if (candidate.age === 0 || candidate.age === 1 && candidate.steps < 2
          ...
          if (improvement && improvement.analysis.maximumAdvantage > analysis.maximumAdvantage + EVOLUTION_RULES.preliminaryAdvantage) {
            candidate.genome = { ...improvement.trial.genome }; candidate.steps += improvement.trial.steps;
      ...
      const used = new Set([genomeKey(record.genome), ...evaluated.map(item => genomeKey(item.candidate.genome))]);
      {
        ...
        for (const { trial, analysis } of proposals) {
          ...
          const candidate = { id: `direction-${state.nextCandidate++}`, genome: { ...trial.genome },
            originDay: environmentDay, lastEvaluation: environmentDay, age: 0, steps: trial.steps,
  ...
  function buildObservation(options, community) {
    ...
    for (const record of state.species) {
      ...
      for (const row of rows) {
        ...
        const morphology = derived.morphology;
        const key = `${derived.role}|${energySources.join(',')}|${derived.size}|${row.habitat}|${mobile}|${morphology.form}|${morphology.pattern}|${morphology.social}`;
        if (!hex.display.has(key)) hex.display.set(key, { role: derived.role, size: derived.size,
          energySources: [...energySources], morphology: { ...morphology }, mobile, habitat: row.habitat, population: 0 });
   ...
   * transports opaque jobs; all partition contents and ecology stay in V4.
